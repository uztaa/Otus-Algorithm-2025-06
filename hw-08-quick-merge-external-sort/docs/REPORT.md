# Отчет о выполненной работе ДЗ-08 (Quick, Merge, External sort)
...
Путь к тестовой папке ROOT_DIR, где будут создаваться/удаляться файлы задается в
...
Переписал сортировщики из прошлого ДЗ-06 на новый интерфейс.
Добавл их в бенчмарк итоговый (SENIOR).

ДЗ-07 (Пирамидальная сортировка) случайно пропустил. Будет реализовано позже.
...
При настройки продолжительности бенчмарка (TIMEOUT_NS) за 1 час получаем такую картину:
